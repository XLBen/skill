...
工作原理一句话：**计划来自论文，不来自现场发挥；验收不跑不算完成。**

→ 每步只做该步的事，越界发现记"待议"，不顺手做
→ 语义不变的小缺口就地补步；设计硬伤才回炉
→ 状态只在 PLAN.md / build-log.md，会话死了能复活
...
- [ ] 5. 全部打勾 → 竣工报告 + 复盘（读 references/retro-protocol.md，用户可跳过）+ 交接块（§5）
```
...
- **轻量差距吸收**：论文没说错、只是计划漏了一小步（语义/边界/验收全不变）→ 按 step-protocol《轻量差距吸收》追加步骤，不走变更单；拿不准就当变更单处理。
...
对照表生成规则：先从 `docs/thesis.md` 枚举**全部** P-xx 与 V-xx 条目，再按每步"来源"字段机械汇总落位——**覆盖必须完整**，没有任何步骤落实的条目显式列"未落实 + 原因"（如"经 CR-01 作废"），不许静默省略。差距吸收追加的步骤也计入对照（来源标"差距吸收"）。
...
竣工报告同时附：**待议清单**（EXECUTE 中累积的全部"待议"条目及去向）、遗留变更单（如有）、建议的后续动作。然后按 `references/retro-protocol.md` 做一次竣工复盘（输入只读 defense-log / build-log / 变更单；产出章程增补建议等三张清单；用户说"不复盘"可跳过），最后输出交接块：
...
- 复盘：<见 build-log《复盘》节，n 条章程增补建议 / 已跳过>
```
...
| 执行步骤 / 验收失败分流 / 轻量差距吸收 / 变更单 CR-xx / 防假打勾 / 恢复细则 | `references/step-protocol.md` |
| 竣工复盘 / 章程增补建议 | `references/retro-protocol.md` |
