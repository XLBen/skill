...
For each new v0.2 acceptance V, preserve the following in the test manifest or
build log, with `given`, `when`, `then`, `assertion_kind` and
`empty_result_policy` also present on the canonical non-human V itself:
...
Automated step gate evidence must come from `verify-step` in the package's
`evidence/` directory, not a handwritten log/hash or `record` pass event.
