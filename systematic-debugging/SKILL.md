...
- 不能稳定复现时，先收敛条件（数据、时序、并发、状态）并保留间歇性原始
  证据；不能复现不等于问题不存在，证据不足时保持未确认而非猜测修复。
...
- 找相邻的正常路径或旧版本，列出失败路径相对它的关键差异；跨组件时按
  输入→边界→输出回读逐段定位，深层错误沿调用链向上追到错误值的来源。
- 断言当前认知：“失败发生在 X 与 Y 之间，因为 Z 已被排除（证据：…）”。

### 3. Hypothesize（假设）

- 有多个合理根因时列出能区分的假设与最小安全探针；如果错误文本、
  正常路径对照或数据流已唯一定位根因，引用证据即可，不为凑两个假设
  虚构竞争解释。第二次同签名失败按 PUA L1 强制换证伪方法。
- 修复前执行必要的区分探针；副作用未知先回读状态，不为复现重放危险动作。
...
- 修复后：原始复现命令转绿 + 受影响范围回归 + 边界条件抽查
  （空输入、重复执行、失败中途恢复）。
- 行为缺陷能稳定自动断言时，用最小回归测试固化根因；局部可逆改动或
  无法自动化的外部边界保留可重放复现与实测回读，不写 tautology。观察
  （product-observer）产出的发现是合法输入：其 reproduction 记录就是
  复现起点；无法立即自动断言的重大问题以可重放证据保留，不得因此
  降级；间歇性发现按 finding-rules 保留待查。修复产生新候选版本并
  触发全产品重新观察（见 mvp-delivery 的 product-audit gate）。
...
诊断结论一行：`根因=<已证实的一句事实或未确认> | 证据=<探针/日志引用> |
修复=<最小变更或待证伪> | 回归=<命令与结果或阻塞原因>`。交给既有 handoff，
不新增工件；没有证据不把假设写成已解决根因。
