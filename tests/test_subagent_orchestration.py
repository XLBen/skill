...
class EntryIntegrationTests(unittest.TestCase):
    ...
    def test_construction_and_reviewer_protocol_map_to_project_agents(self):
        ...
        self.assertIn("references/seat-dispatch.md", construction)
        seat_dispatch = (ROOT / "construction/references/seat-dispatch.md").read_text(encoding="utf-8")
        self.assertIn("`direct` profile overrides interaction settings", seat_dispatch)
