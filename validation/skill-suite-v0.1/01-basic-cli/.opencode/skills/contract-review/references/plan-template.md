...
For a new v0.1 project, `/plan` is allowed only after contract-review has
...
After a slice is accepted, planned expansion is compiled from an SI delta and
gets a new contract/PLAN revision. Do not edit a completed PLAN or create a CR
merely to add an already-planned story. Existing CR recovery remains scoped to
the typed impact closure of an actual contract/reality mismatch.
