...
The engine's historical schema still requires `expected`, but v0.1 contract
review adds a stronger prompt-layer rule for every new non-human V:
...
```json
{
  ...
  "assertion_kind": "content | invariant | schema | count | user-visible"
}
```
...
`expected: exit 0` without these fields is not a valid v0.1 acceptance, even
though the unchanged engine may parse it for legacy compatibility.
