...
- In a v0.1 first-slice audit, verify that the Phase 0 record exists and that
...
- In a step or converge audit, inspect the test-author manifest and working
  ...
  different subagent/session IDs, the frozen test hash is unchanged, the red
  result precedes implementation, and the final V contains a content/state
  assertion rather than only exit 0.
