...
先区分新目标与恢复目标。无参数时从持久状态恢复唯一活动目标；多个候选无法消歧才询问。检查当前会话和项目事实，避免重复询问已有答案。

当目标不清楚时，加载 `grill` 并使用 **Work Inference Mode**：
...
5. 用简短的内部 `WORKING_GOAL` 总结原始目标、必需结果、明确约束、推断假设、完成证据和下一步，然后立即行动。
...
技能目录不是固定白名单。目标建立时、进入新的技术/验收边界时、方案受挫时及最终验收前，按 `../mvp-delivery/references/subagent-orchestration.md` 的 **Skill Applicability Selection** 执行有界选择：
...
3. 阅读候选 skill 的 description 与适用规则，加载适用者，实际执行其职责；对不适用者不加载并简记原因。不要为了“调用全部 skill”运行与目标无关的流程。
...
- 有界、可逆且目标明确的小改动可直接做，并进行与风险相称的真实验证；不为其强制创建 goal、brief 或 plan 文件。
- 多步骤、跨模块或高不确定工作加载 `writing-plans` 制定可执行路线。目标变化或设计假设被证伪时，带新证据重规划，然后由本 `/work` 循环自动恢复执行；重规划本身不要求用户重新发命令。
