...
从已确认的 grill brief、明确用户目标或无命令 work 工作目标综合出目标用户、
主流程、输入输出、必需功能、非目标和约束。给正常/失败的具体场景。
有 brief 时保留其 ID → outcome ID；无 brief 时按原始请求追踪，不伪造
brief ID 或用户确认，不把问题列表直接改成任务。
...
需求 → 实现任务 → 组件/真实边界/旅程验证的表。挑出少数最可能漏掉的
真实输入/状态，逐项落实到任务检查或标明不适用，不为固定数量造测试。
程序只核结构关联，规划者需确认
